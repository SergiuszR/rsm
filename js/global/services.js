...
$(document).ready(function() {
    ...
    function initServicesAnimation() {
        // Check if we're on desktop (>= 992px) for CSS transforms
        ...
        // Only apply CSS transforms on desktop
        if (isDesktop) {
        ...
        // Show first video initially with smooth reveal (desktop only)
        if (isDesktop && videoContainers.length > 0) {
        ...
        // Create scroll animation for each service item (desktop only)
        if (isDesktop) {
            servicesItems.forEach((item, index) => {
        ...
        // Handle window resize to reinitialize on breakpoint changes
        let resizeTimeout;
        window.addEventListener('resize', function() {
            ...
            resizeTimeout = setTimeout(function() {
                ...
                // If breakpoint changed, refresh ScrollTriggers
                if (newIsDesktop !== isDesktop) {
                    ScrollTrigger.refresh();
                    console.log('Services animation: Breakpoint changed, ScrollTriggers refreshed');
                }
